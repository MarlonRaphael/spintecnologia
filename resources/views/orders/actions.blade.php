<div class="row mx-2">
  <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
    @if($model->isOpened())
      <a href="{{ route('orders.pay', $model->id) }}" class="btn btn-success btn-sm" id="btn-paid">
        <i class="fas fa-check fa-sm"></i>
      </a>
    @endif
    @if(!$model->isCanceled())
        <a href="{{ route('orders.cancel', $model->id) }}" class="btn btn-danger btn-sm small">
          <i class="fas fa-ban fa-sm"></i>
        </a>
    @endif
    <button aria-expanded="false" aria-haspopup="true" class="btn ripple btn-outline-secondary btn-sm"
    ...
    <div class="dropdown-menu tx-13 shadow-sm" x-placement="bottom-start"
      ...
      @endif
      @if (Route::has(resource_url($model, 'update')) && $model->isOpened())
